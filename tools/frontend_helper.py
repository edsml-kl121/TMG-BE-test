...
def initialize_db_client(host=milvus_host, port=milvus_port):
    ...
    """
    return connections.connect("default", host="158.175.177.136", port="8080", secure=True, server_pem_path="./cert.pem", server_name="localhost",user="root",password="4XYg2XK6sMU4UuBEjHq4EhYE8mSFO3Qq")
...
def get_db_results(query_text, model, collection_name="promotion_collection_scb", n_results=4):
    ...
    documents = collection.search(data=query_encode, anns_field="embeddings", param={"metric":"IP","offset":0},
                    output_fields=["new_translated_docs", "page_content", "sources", "pagesno"], limit=n_results)
    return documents[0]
